...
outTxt_align_press = 'Align_press_' + outfile + '.txt'
...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    for p in range(start, end):     
        ...
        fa_all_tot = np.array([])#[[0 for b in range(NBins)] for a in range(NBins)]
        r_dist_tot = np.array([])#[[0 for b in range(NBins)] for a in range(NBins)]
        activity_all=np.array([])
        binx_all=np.array([])
        biny_all=np.array([])
        partid_all=np.array([])
                                    ...
                                    if ext_edge_id[ix][iy]==0:
                                        ...
                                        if len(binParts[ix][iy])>0:
                                            for h in range(0, len(binParts[ix][iy])):
                                                ...
                                                new_align_num_trad[ix][iy]+= 1
                                                if typ[binParts[ix][iy][h]]==0:
                                                    fa_all_tot=np.append(fa_all_tot, r_dot_p*peA)
                                                    r_dist_tot = np.append(r_dist_tot, difr_short)
                                                    activity_all=np.append(activity_all, peA)
                                                    partid_all=np.append(partid_all, binParts[ix][iy][h])
                                                else:
                                                    fa_all_tot=np.append(fa_all_tot, r_dot_p*peB)
                                                    r_dist_tot = np.append(r_dist_tot, difr_short)
                                                    activity_all=np.append(activity_all, peB)
                                                    partid_all=np.append(partid_all, binParts[ix][iy][h])
                                                if typ[binParts[ix][iy][h]]==0:
        ...
        if gas_num_dens_num > 0:
            ...
            gas_avg = 0
        interior_rad = np.max(bub_width)-np.max(edge_width_arr)
        ...
        if np.floor(np.min(r_dist_tot))<= 0.0:  
                                                 
            xint = np.linspace(0, np.ceil(np.max(r_dist_tot)), num=int((int(np.ceil(np.ceil(np.max(r_dist_tot))))+1)))
            ...
            for i in range(1, len(xint)):
                ...
                points = np.where((min_range<=r_dist_tot) & (r_dist_tot<=max_range))[0]  
                if len(points)>0:
                    points2 = np.logical_not(np.isnan(fa_all_tot[points]))
                    if len(points2)>0:
                        yfinal[i-1] = np.sum(fa_all_tot[points][points2])
                        if len(surface_area_ext)==0:
            ...
            for o in range(1, len(xint_final)):
                int_sum += ((xint_final[o]-xint_final[o-1])/2)*(yint_final[o]+yint_final[o-1])
        else:
            int_sum=0
        ...
        g.write('{0:.6f}'.format(int_sum).center(20) + '\n')
        g.close()
