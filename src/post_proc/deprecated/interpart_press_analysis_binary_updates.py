...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    for p in range(start, end):
        ...
        bulk_x_force = np.array([])
        bulk_y_force = np.array([])
        ...
        for i in all_bulk_nlist.point_indices:
            if i not in bulk_lat_ind:
                ...
                for k in loc:
                    fx, fy = computeFLJ(rijs[k,0], rijs[k,1], eps)
                    totxForce += fx
                    totyForce += fy
                    bulkSigXX += (fx * rijs[k,0])
                    bulkSigYY += (fy * rijs[k,1])
                    bulkSigXY += (fx * rijs[k,1])
                    bulkSigYX += (fy * rijs[k,0])
                bulk_x_force = np.append(bulk_x_force, totxForce)
                bulk_y_force = np.append(bulk_y_force, totyForce)
        ...
        int_x_force = np.array([])
        int_y_force = np.array([])
        ...
        for i in all_int_nlist.point_indices:
            if i not in int_lat_ind:
                ...
                for k in loc:
                    ...
                    totxForce += fx
                    totyForce += fy
                    intSigXX += (fx * rijs[k,0])
                    intSigYY += (fy * rijs[k,1])
                    intSigXY += (fx * rijs[k,1])
                    intSigYX += (fy * rijs[k,0])
                int_x_force = np.append(int_x_force, totxForce)
                int_y_force = np.append(int_y_force, totyForce)
                int_lat_ind = np.append(int_lat_ind, int(i))


        dense_x_force = np.append(bulk_x_force, int_x_force)
        dense_y_force = np.append(bulk_y_force, int_y_force)
        dense_force_mag = (dense_x_force**2 + dense_y_force**2)**0.5
        ...
        dense_force_mean = np.mean(dense_force_mag)
        lat_theory2 = conForRClust(pe_net_int-50, eps)

        fig = plt.figure(figsize=(7,6))
        ax = fig.add_subplot(111)
        im = plt.scatter(pos_bulk_int_x_lat+h_box, pos_bulk_int_y_lat+h_box, c=dense_force_mag, s=0.7, vmin=0.5*dense_force_mean, vmax=2.0*dense_force_mean)


        min_n = 0.5*dense_force_mean
        max_n = 2.0*dense_force_mean
        ...
        plt.text(0.75, 0.92, s=r'$\overline{a}$' + ' = ' + '{:.3f}'.format(dense_force_mean),
                fontsize=18, transform = ax.transAxes,
                bbox=dict(facecolor=(1,1,1,0.75), edgecolor=(0,0,0,1), boxstyle='round, pad=0.1'))
