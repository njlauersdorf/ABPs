...
import numpy as np
import matplotlib
#matplotlib.use('Agg') #Uncomment only when running on cluster
...
outPath2='/Volumes/External/test_video_mono/'
outPath='/Volumes/External/test_video_mono/'


#Run on Cluster
#hoomdPath='/nas/home/njlauers/hoomd-blue/build/'
#outPath2='/proj/dklotsalab/users/ABPs/binary_soft/random_init/final_runs/all_N_100000/eps1.0/text_files2/'
#outPath='/proj/dklotsalab/users/ABPs/binary_soft/random_init/final_runs/all_N_100000/eps1.0/picture_files2/'
...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    start = 470                                               # first frame to process
    ...
    time_arr=np.zeros(end)                                  #time step array
    ...
    for j in range(start, int(end)):     
        ...
        for ix in range(0, len(occParts)):
                for iy in range(0, len(occParts)):
                    if j>start:
                        vel_mag[ix][iy] = ((v_avg_x[ix][iy]**2+v_avg_y[ix][iy]**2)**0.5)/peB    #Average velocity per bin of all particles relative to largest preferred velocity (peB)
                        vel_magA[ix][iy] = ((v_avg_xA[ix][iy]**2+v_avg_yA[ix][iy]**2)**0.5)/peA #Average velocity per bin of type A particles relative to preferred velocity (peA)
                        vel_magB[ix][iy] = ((v_avg_xB[ix][iy]**2+v_avg_yB[ix][iy]**2)**0.5)/peB #Average velocity per bin of type B particles relative to preferred velocity (peB)
                        vel_magDif[ix][iy] = (vel_magB[ix][iy]*peB-vel_magA[ix][iy]*peA)        #Difference in magnitude of average velocity per bin between type B and A particles
        ...
        #Individually plot each particle's position and color code by the unique
        #phase/structure they belong to
        ...
        plt.scatter(pos[int_id_plot,0]+h_box, pos[int_id_plot,1]+h_box, s=1, marker='.', c=yellow)
        ...
        plt.legend(handles=[green_patch, yellow_patch, red_patch], fancybox=True, framealpha=0.75, ncol=1, fontsize=12, loc='upper left',labelspacing=0.1, handletextpad=0.1)
        ...
        plt.tight_layout()
        plt.savefig(outPath + 'interface_acc2_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make vector plot of average orientation per bin on top of the particle locations
        #individually plotted and color coded by the unique phase/structure they belong to
        fig = plt.figure(figsize=(6.5, 6))
        ...
        #Make heatmap of the number density of particle types A and B
        ...
        clb.ax.set_title(r'$n$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'density_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make heatmap of the number density of particle type A
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, num_dens3A, vmin=0.0, vmax=vmax_eps)
        ...
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$n_\mathrm{A}$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'densityA_' + out + pad + ".png", dpi=150)
        plt.close()
 
        #Make heatmap of the number density of particle type B      
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, num_dens3B, vmin=0.0, vmax=vmax_eps)
        ...
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$n_\mathrm{B}$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'densityB_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make heatmap of the difference in magnitude of the number density of particle types B and A
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, num_densDif, vmin=-vmax_eps, vmax=vmax_eps, cmap='seismic')
            
        norm= matplotlib.colors.Normalize(vmin=-vmax_eps, vmax=vmax_eps)

        sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
        sm.set_array([])
        tick_lev = np.arange(-vmax_eps, vmax_eps+vmax_eps/10, vmax_eps/10)
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$n_\mathrm{B}-n_\mathrm{A}$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'densityDif_' + out + pad + ".png", dpi=150)
        plt.close()
        
        
        #Make heatmap of the pressure of particle types A and B
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, press_bin, vmin=0.0, vmax=vmax_eps*0.5)
            
        norm= matplotlib.colors.Normalize(vmin=0.0, vmax=vmax_eps*0.5)

        sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
        sm.set_array([])
        tick_lev = np.arange(0, vmax_eps*0.5+(vmax_eps*0.5), (vmax_eps*0.5)/10)
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$\alpha \times n$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'pressure_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make heatmap of the pressure of particle type A
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, press_binA, vmin=0.0, vmax=vmax_eps*0.5)
            
        norm= matplotlib.colors.Normalize(vmin=0.0, vmax=vmax_eps*0.5)

        sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
        sm.set_array([])
        tick_lev = np.arange(0, (vmax_eps*0.5)+(vmax_eps*0.5)/10, (vmax_eps*0.5)/10)
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$\alpha \times n$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'pressureA_' + out + pad + ".png", dpi=150)
        plt.close()

        #Make heatmap of the pressure of particle type B               
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, press_binB, vmin=0.0, vmax=vmax_eps*0.5)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'pressureB_' + out + pad + ".png", dpi=150)
        plt.close()

        #Make heatmap of the difference in magnitude of pressure between particle type B and A        
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, press_binDif, vmin=-vmax_eps*0.5, vmax=vmax_eps*0.5, cmap='seismic')
            
        norm= matplotlib.colors.Normalize(vmin=-vmax_eps*0.5, vmax=vmax_eps*0.5)

        sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
        sm.set_array([])
        tick_lev = np.arange(-vmax_eps*0.5, vmax_eps*0.5+(vmax_eps*0.5)/10, (vmax_eps)/10)
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$\alpha \times n$', fontsize=18)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'pressureDif_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make vector plot of the average orientation per bin of particle types A and B
        fig = plt.figure(figsize=(6.5, 6))

        myEps = [1., 0.1, 0.01, 0.001, 0.0001]

        plt.quiver(pos_box_x, pos_box_y, align_avg_x, align_avg_y, color='black')
        plt.xticks(pos_box_start)
        plt.yticks(pos_box_start)
        plt.tick_params(
                                axis='x',          # changes apply to the x-axis
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'alignment_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make vector plot of the average orientation per bin of particle type A
        fig = plt.figure(figsize=(6.5, 6))

        myEps = [1., 0.1, 0.01, 0.001, 0.0001]

        plt.quiver(pos_box_x, pos_box_y, align_avg_xA, align_avg_yA, color='black')
        plt.xticks(pos_box_start)
        plt.yticks(pos_box_start)
        plt.tick_params(
                                axis='x',          # changes apply to the x-axis
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'alignmentA_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make vector plot of the average orientation per bin of particle type B
        fig = plt.figure(figsize=(6.5, 6))

        myEps = [1., 0.1, 0.01, 0.001, 0.0001]

        plt.quiver(pos_box_x, pos_box_y, align_avg_xB, align_avg_yB, color='black')
        plt.xticks(pos_box_start)
        plt.yticks(pos_box_start)
        plt.tick_params(
                                axis='x',          # changes apply to the x-axis
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'alignmentB_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #Make heatmap of the difference in magnitude of orientation between particle type B and A
        fig = plt.figure(figsize=(8,6))
        im = plt.contourf(pos_box_x, pos_box_y, align_magDif, vmin=-0.5, vmax=0.5, cmap='seismic')
            
        norm= matplotlib.colors.Normalize(vmin=-0.5, vmax=0.5)

        sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
        sm.set_array([])
        tick_lev = np.arange(-0.5, 0.5+0.5/10, 0.5/5)
        clb = fig.colorbar(sm, ticks=tick_lev)
        clb.ax.tick_params(labelsize=16)
        clb.ax.set_title(r'$|\alpha_\mathrm{B}-\alpha_\mathrm{A}|$', fontsize=16)
        ...
        plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
        plt.tight_layout()
        plt.savefig(outPath + 'alignmentDif_' + out + pad + ".png", dpi=150)
        plt.close()
        
        #If at least one time-frame already analyzed (need previous position)
        if j>start:
            
            #Make vector plot of the velocity of both particle types excluding the gas phase
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]

            plt.quiver(pos_box_x, pos_box_y, v_avg_x_no_gas, v_avg_y_no_gas, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocitynog_' + out + pad + ".png", dpi=150)
            plt.close()
            
            #Make vector plot of the velocity of particle type A excluding the gas phase
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]

            plt.quiver(pos_box_x, pos_box_y, v_avg_xA_no_gas, v_avg_yA_no_gas, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocityAnog_' + out + pad + ".png", dpi=150)
            plt.close()
            
            #Make vector plot of the velocity of particle type B excluding the gas phase
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]

            plt.quiver(pos_box_x, pos_box_y, v_avg_xB_no_gas, v_avg_yB_no_gas, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocityBnog_' + out + pad + ".png", dpi=150)
            plt.close()
            
            
            #Make vector plot of the velocity of both particle types
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]

            plt.quiver(pos_box_x, pos_box_y, v_avg_x, v_avg_y, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            #Make vector plot of the velocity of particle type A
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]

            plt.quiver(pos_box_x, pos_box_y, v_avg_xA, v_avg_yA, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            #Make vector plot of the velocity of particle type B
            fig = plt.figure(figsize=(6.5, 6))

            myEps = [1., 0.1, 0.01, 0.001, 0.0001]
            
            plt.quiver(pos_box_x, pos_box_y, v_avg_xB, v_avg_yB, color='black')
            plt.xticks(pos_box_start)
            plt.yticks(pos_box_start)
            plt.tick_params(
                                    axis='x',          # changes apply to the x-axis
            ...
            #Make heatmap of the difference in magnitude of velocity between particle type B and A
            fig = plt.figure(figsize=(8,6))
            im = plt.contourf(pos_box_x, pos_box_y, vel_magDif, vmin=-peB, vmax=peB, cmap='seismic')
                
            norm= matplotlib.colors.Normalize(vmin=-peB, vmax=peB)
    
            sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
            sm.set_array([])
            tick_lev = np.arange(-peB, peB+peB/10, peB/5)
            clb = fig.colorbar(sm, ticks=tick_lev)
            clb.ax.tick_params(labelsize=16)
            clb.ax.set_title(r'$|v_\mathrm{B}-v_\mathrm{A}|$', fontsize=16)
            ...
            #Make heatmap of the magnitude in velocity of particle types A and B
            fig = plt.figure(figsize=(8,6))
            im = plt.contourf(pos_box_x, pos_box_y, vel_mag, vmin=0.0, vmax=1.0)
                
            norm= matplotlib.colors.Normalize(vmin=0.0, vmax=1.0)
    
            sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
            sm.set_array([])
            tick_lev = np.arange(0.0, 1.0+1.0/10, 1.0/10)
            clb = fig.colorbar(sm, ticks=tick_lev)
            clb.ax.tick_params(labelsize=16)
            clb.ax.set_title(r'$|v|/\mathrm{Pe}_\mathrm{B}$', fontsize=16)
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocityMag_' + out + pad + ".png", dpi=150)
            plt.close()
            
            #Make heatmap of the magnitude in veocity of particle type A
            fig = plt.figure(figsize=(8,6))
            im = plt.contourf(pos_box_x, pos_box_y, vel_magA, vmin=0.0, vmax=1.0)
            ...
            clb = fig.colorbar(sm, ticks=tick_lev)
            clb.ax.tick_params(labelsize=16)
            clb.ax.set_title(r'$|v_\mathrm{A}|/\mathrm{Pe}_\mathrm{A}$', fontsize=16)
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocityMagA_' + out + pad + ".png", dpi=150)
            plt.close()
            
            #Make heatmap of the magnitude in veocity of particle type B
            fig = plt.figure(figsize=(8,6))
            im = plt.contourf(pos_box_x, pos_box_y, vel_magB, vmin=0.0, vmax=1.0)
            ...
            clb = fig.colorbar(sm, ticks=tick_lev)
            clb.ax.tick_params(labelsize=16)
            clb.ax.set_title(r'$|v_\mathrm{B}|/\mathrm{Pe}_\mathrm{B}$', fontsize=16)
            ...
            plt.title(r'$\mathrm{Pe}_\mathrm{A}$' + ' = ' + str(int(peA)) + ', ' + r'$\mathrm{Pe}_\mathrm{B}$' + ' = ' + str(int(peB)) + ', ' + r'$\chi_\mathrm{A}$' + ' = ' + str(parFrac/100) + ', ' + r'$\phi$' + ' = ' + str(phi) + ', ' + r'$\epsilon$' + ' = ' + r'$10^{{{}}}$'.format(int(np.log10(eps))), fontsize=17)
            plt.tight_layout()
            plt.savefig(outPath + 'velocityMagB_' + out + pad + ".png", dpi=150)
            plt.close()
