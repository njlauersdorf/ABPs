...
outPath = current_path + '/dif_avg/'

current_files = os.listdir(outPath)
...
for i in range(0, len(current_files)):
    ...
    if current_files[i][0:6] == 'dif_av':
        ...
        file_path = outPath + current_file
        ...
        if len(test_id_new)>20:
            ...
            for j in range(0, num_time_steps):
                print(df['tauB'].values[start_val])
                print(start_val)
            ...
            def write_to_txt(input_dict, outPath):
                ...
                headers = ['pa', 'pb', 'xa']
                data = [inFile[pa_id+2:pb_id-1], inFile[pb_id+2:xa_id-1], inFile[xa_id+2:xa_id+4]]
            ...
            write_to_txt(averaged_data_arr, current_path + '/compiled_heterogeneity_dif_av.txt')
...
outPath = current_path + '/dif_Ra/'

current_files = os.listdir(outPath)

for i in range(0, len(current_files)):

    if current_files[i][0:6] == 'dif_Ra':
        ...
        if len(test_id_new)>20:
            ...
            write_to_txt(averaged_data_arr, current_path + '/compiled_heterogeneity_dif_Ra.txt')