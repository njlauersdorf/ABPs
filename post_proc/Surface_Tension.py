...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/surfacetens/'
...
outTxt = 'surface_tension_' + outF + '.txt'
...
g.write('tauB'.center(15) + ' ' +\
        ...
        'lat'.center(15) + ' ' +\
        'cluster_radius'.center(15) + ' ' +\
        'edge_area'.center(15) + ' ' +\
        'gas_area'.center(15) + ' ' +\
        'bulk_area'.center(15) + ' ' +\
        'gasPress'.center(15) + ' ' +\
        'bulkPress'.center(15) + ' ' +\
        'surfaceTense'.center(15) + '\n')
g.close()
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            if steps>0:
                    ...
                    # Loop through surrounding y-index
                        ...
                        # Loop through all particles in that bin
                            ...
                            # If potential is on ...
                                ...
                                # This is the work done by the interparticle force between bulk particles
                                        if clust_size[ids[k]] >= min_size:
                                            bulkSigXX += (fx * (pos[comp][0] - refx))
                                            bulkSigYY += (fy * (pos[comp][1] - refy))
                                # This goes into the gas pressure
                                # This is the total work done by the interparticle force between gas particles
                                        else:
                ...
                edge_circumference=sampleCircle.length
                edge_dens=((count_A_edge_final+count_B_edge_final)/edge_area)
                edge_width_total=(bulk_r_lim_final+2*lat)
                ...
                bulk_circumference=sampleCircle.length
                
                bulk_dens=((count_A_dense_final+count_B_dense_final)/bulk_area)
                bulk_radius=(bulk_area/math.pi)**0.5
                cluster_circum=(bulk_radius+edge_width_total)*2*math.pi
                cluster_radius=edge_width_total+bulk_radius
            #BLUE = '#6699cc'
            #fig = plt.figure() 
            #ax = fig.gca() 
            ...
            #plt.show()
                ...
                gasPress = gasTrace / gas_area
                if bulk_area > 0:
                    bulkPress = bulkTrace / bulk_area
                    # Surface tension
                    surfaceTense = (bulkPress-gasPress)*cluster_radius/2#(bulkPress * bulk_radius - gasPress * cluster_radius)
                else:
                    bulkPress = 0.
                    surfaceTense = 0.
                
                
                print("Gas phase area: ", gas_area)
                print("Bulk phase area: ", bulk_area)
                ...
                print("Cluster circumference: ", cluster_circum)
                ...
                g.write('{0:.6f}'.format(cluster_radius).center(15) + ' ')
                g.write('{0:.6f}'.format(edge_area).center(15) + ' ')
                g.write('{0:.6f}'.format(gas_area).center(15) + ' ')
                g.write('{0:.6f}'.format(bulk_area).center(15) + ' ')
                g.write('{0:.6f}'.format(gasPress).center(15) + ' ')
                g.write('{0:.6f}'.format(bulkPress).center(15) + ' ')
                g.write('{0:.6f}'.format(surfaceTense).center(15) + '\n')
                g.close()