...
imgPath='/pine/scr/n/j/njlauers/scm_tmpdir/edge_dist/'
r_cut=2**(1/6)
# Get infile and open
...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/edge_dist/'#'Users/nicklauersdorf/hoomd-blue/build/04_01_20_parent/'#'/Volumes/External/04_01_20_parent/gsd/'
outF = inFile[:-4]
...
outTxt = 'edge_dist_' + outF + '.txt'
g = open(outPath+outTxt, 'w+') # write file headings
g.write('tauB'.center(30) + ' ' +\
        'total_edge_distance'.center(30) + ' ' +\
        'total_edge_bins'.center(30) + '\n')
g.close()
...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    for j in range(start, int(end)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
            ...
            for ix in range(0, len(occParts)):
                for iy in range(0, len(occParts)):
                    if testIDs[ix][iy]==0:
                            ...
                            y_dis=np.abs(com_tmp_posY-y_mid)
        ...
        g.write('{0:.6f}'.format(edge_dist).center(30) + ' ')
        g.write('{0:.6f}'.format(edge_bins).center(30) + '\n')
        g.close()
