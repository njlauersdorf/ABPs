...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/radial_phase_dens_updated/'#'Users/nicklauersdorf/hoomd-blue/build/04_01_20_parent/'#'/Volumes/External/04_01_20_parent/gsd/'
...
outTxt = 'radial_phase_dens_' + outF + '.txt'
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            if steps>0:
                ...
                tmp_pos=np.zeros((len(pos),2))
            
                count_A_edge=np.zeros(len(r))
                count_B_edge=np.zeros(len(r))
                count_A_gas=np.zeros(len(r))
                count_B_gas=np.zeros(len(r))
                count_A_bulk=np.zeros(len(r))
                count_B_bulk=np.zeros(len(r))
            
                tmp_pos[:,0] = pos[:,0] - com_x
                tmp_pos[:,1] = pos[:,1] - com_y
            
                for x in range(0,len(tmp_pos)):
                    if tmp_pos[x,0]>h_box:
                        tmp_pos[x,0]=tmp_pos[x,0]-l_box
                    elif tmp_pos[x,0]<(-h_box):
                        tmp_pos[x,0]=tmp_pos[x,0]+l_box
                    if tmp_pos[x,1]>h_box:
                        tmp_pos[x,1]=tmp_pos[x,1]-l_box
                    elif tmp_pos[x,1]<(-h_box):
                        tmp_pos[x,1]=tmp_pos[x,1]+l_box

                r_dist_com=(tmp_pos[:,0]**2+tmp_pos[:,1]**2)**0.5
            
                for i in range(0,len(r)):
                    parts_in=np.where(((r[i]-1)<=r_dist_com) & (r_dist_com<=r[i]))[0]
                    for k in parts_in:
                        if PhaseParts_final[k]==0:
                            if typ[k]==0:
                                count_A_edge[i]+=1
                            elif typ[k]==1:
                                count_B_edge[i]+=1
                        elif PhaseParts_final[k]==1:
                            if typ[k]==0:
                                count_A_bulk[i]+=1
                            elif typ[k]==1:
                                count_B_bulk[i]+=1
                        elif PhaseParts_final[k]==2:
                            if typ[k]==0:
                                count_A_gas[i]+=1
                            elif typ[k]==1:
                                count_B_gas[i]+=1
                ...
                for i in range(0,len(r)):
                    count_A_edge[i]=count_A_edge[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))
                    count_B_edge[i]=count_B_edge[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))
                    count_A_gas[i]=count_A_gas[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))
                    count_B_gas[i]=count_B_gas[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))
                    count_A_bulk[i]=count_A_bulk[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))
                    count_B_bulk[i]=count_B_bulk[i]/(np.pi*((r[i]**2)-((r[i]-1)**2)))

                g = open(outPath+outTxt, 'a')
                for i in range(0,len(r)):
                    g.write('{0:.1f}'.format(tst).center(15) + ' ')
                    g.write('{0:.3f}'.format(r[i]).center(15) + ' ')
                    g.write('{0:.3f}'.format(r[i]-1).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_A_edge[i]).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_B_edge[i]).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_A_gas[i]).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_B_gas[i]).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_A_bulk[i]).center(15) + ' ')
                    g.write('{0:.6f}'.format(count_B_bulk[i]).center(15) + ' ')
                    g.write('{0:.1f}'.format(bulk_particle_range_final).center(15) + ' ')
                    g.write('{0:.1f}'.format(gas_particle_range).center(15) + ' ')
