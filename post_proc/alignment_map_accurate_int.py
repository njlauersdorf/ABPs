       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            for k in range(1,len(num_sights)):
                        ...
                        if c!=0:
                            ...
                            if d!=0:
                                ...
                                if begin_rad_point<end_rad_point:
                                    ...
                                    print('start')
                                    print(len(norm_rad_very_orig))
                                    print(len(num_dens_very_orig))
                                    print(len(align_very_orig))
                                    ...
                                    print(len(norm_rad_very_orig_app))
                                    print(len(num_dens_very_orig_app))
                                    print(len(align_very_orig_app))
                ...
                int_width_num += 1
            search_range = int_width_vals/int_width_num
            ...
            for ix in range(0, len(binParts)):
                for iy in range(0, len(binParts)):
                    if losBin[ix][iy]==1:
                            ...
                            x_ref2=(ix+0.5)*sizeBin
                            y_ref2=(iy+0.5)*sizeBin
          
                            difx = np.abs(x_ref2 - com_tmp_posX)
                            ...
                            dify = np.abs(y_ref2 - com_tmp_posY)
                                                            
                            difx_abs = np.abs(dify)
                            if difx_abs>=h_box:
                                if dify < -h_box:
                                    dify += l_box
                                else:
                            ...
                            if (difx)>0:
                                ...
                                if (dify)>0:
                                    ...
                                    part_angle=(num_sights[k]+num_sights[k-1])/2
                            r_min=(difx**2+dify**2)**0.5
                            if begin_rad_point<=r_min<=end_rad_point:
                                                                
                                for j in range(0, len(binParts[ix][iy])):
                                    PhaseParts2[binParts[ix][iy][j]]=0
            yellow = ("#fdfd96")
            green = ("#77dd77")
            red = ("#ff6961")
            ...
            plt.title(r'$Pe_\mathrm{A}$' + '=' + str(int(peA)))
            plt.tight_layout()
