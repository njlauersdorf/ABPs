...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/rdf_phase_data/'
...
outTxt2 = 'Phase_rdf_' + out + '.txt'
g = open(outPath+outTxt2, 'w') # write file headings
g.write('peA'.center(15) + ' ' +\
        'peB'.center(15) + ' ' +\
        'distance'.center(15) + ' ' +\
        'rdf_all_edge'.center(15) + ' ' +\
        'redf_AA_edge'.center(15) + ' ' +\
        'rdf_AB_edge'.center(15) + ' ' +\
        'rdf_BA_edge'.center(15) + ' ' +\
        'rdf_BB_edge'.center(15) + ' ' +\
        'rdf_all_dense'.center(15) + ' ' +\
        'redf_AA_dense'.center(15) + ' ' +\
        'rdf_AB_dense'.center(15) + ' ' +\
        'rdf_BA_dense'.center(15) + ' ' +\
        'rdf_BB_dense'.center(15) + ' ' +\
        'rdf_all_gas'.center(15) + ' ' +\
        'redf_AA_gas'.center(15) + ' ' +\
        'rdf_AB_gas'.center(15) + ' ' +\
        'rdf_BA_gas'.center(15) + ' ' +\
        'rdf_BB_gas'.center(15) + ' ' +\
        'time_steps'.center(15) + '\n')
g.close()
...
rplot = np.linspace(1.0, rstop, N)    # 0 through searchRange with Nbins
...
with hoomd.open(name=inFile, mode='rb') as t:
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            for ix in range(0, len(occParts)):
                for iy in range(0, len(occParts)):
                    if testIDs[ix][iy]==0:
                        if len(binParts[ix][iy]) != 0:
                            if clust_size[ids[binParts[ix][iy][0]]] == np.amax(clust_size):
                                min_size_y=sizeBin*(iy-1)
                                max_size_y=sizeBin*iy
                                min_size_x=sizeBin*(ix-1)
                                max_size_x=sizeBin*ix
                                x_mid=(min_size_x+max_size_x)/2
                                y_mid=(min_size_y+max_size_y)/2
                                x_dist=np.abs(com_tmp_posX-x_mid)
                                y_dist=np.abs(com_tmp_posY-y_mid)
                                edge_dist=(x_dist**2+y_dist**2)**0.5
                                edge_dist_total+=edge_dist
                                edge_bins+=1
            clust_rad=edge_dist_total/edge_bins
            ...
            steps=0
            if typS==0:
                while count_B_edge*((parFrac)/100)<=count_A_edge*((100.0-parFrac)/100):
                    bulk_r_lim=bulk_particle_range*lat
                    search_range=int(math.ceil((bulk_particle_range)*lat/sizeBin))
                    for ix in range(0, len(occParts)):
                        for iy in range(0, len(occParts)):
                            if edgeBin[ix][iy]==1:
                                testIDs[ix][iy]=0
                    ...
                    slowParts_num=np.where(typ==0)
                    fastParts_num=np.where(typ==1)
                    edgeParts_num_A=np.intersect1d(edgeParts_num, slowParts_num)
                    edgeParts_num_B=np.intersect1d(edgeParts_num, fastParts_num)
                    bulkParts_num_A=np.intersect1d(bulkParts_num, slowParts_num)
                    bulkParts_num_B=np.intersect1d(bulkParts_num, fastParts_num)
                    gasParts_num_A=np.intersect1d(gasParts_num, slowParts_num)
                    gasParts_num_B=np.intersect1d(gasParts_num, fastParts_num)
                    ...
                    gas_pos_A=pos[gasParts_num_A]
                    bulk_pos_A=pos[bulkParts_num_A]
                    edge_pos_A=pos[edgeParts_num_A]
                    gas_pos_B=pos[gasParts_num_B]
                    bulk_pos_B=pos[bulkParts_num_B]
                    edge_pos_B=pos[edgeParts_num_B]
                    ...
                    if count_B_edge*((parFrac)/100)>count_A_edge*((100.0-parFrac)/100): 
                        if steps>0:                           
                            break
                        else:
                            pass
                    
                    if bulk_r_lim>=(20.0*lat):
    ...
    #system_all_cut = freud.AABBQuery(f_box, f_box.wrap(pos_all_in_clust))    #Calculate neighbor list
                #system_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos))    #Calculate neighbor list
                system_dense_cut = freud.AABBQuery(f_box, f_box.wrap(bulk_pos))    #Calculate neighbor list
                system_edge_cut = freud.AABBQuery(f_box, f_box.wrap(edge_pos)) 
                system_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos))
    
    
                #system_A_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos_A))    #Calculate neighbor list
                #system_B_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos_B))    #Calculate neighbor list
                system_A_dense_cut = freud.AABBQuery(f_box, f_box.wrap(bulk_pos_A))    #Calculate neighbor list
                system_B_dense_cut = freud.AABBQuery(f_box, f_box.wrap(bulk_pos_B))    #Calculate neighbor list
                system_A_edge_cut = freud.AABBQuery(f_box, f_box.wrap(edge_pos_A))    #Calculate neighbor list
                system_B_edge_cut = freud.AABBQuery(f_box, f_box.wrap(edge_pos_B))    #Calculate neighbor list
                system_A_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos_A))    #Calculate neighbor list
                system_B_gas_cut = freud.AABBQuery(f_box, f_box.wrap(gas_pos_B))    #Calculate neighbor list



                radialDFall_dense = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFAA_dense = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                print('4')
                radialDFAB_dense = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBA_dense = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBB_dense = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
            
            
                radialDFall_edge = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFAA_edge = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                print('4')
                radialDFAB_edge = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBA_edge = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBB_edge = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
            
                radialDFall_gas = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFAA_gas = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                print('4')
                radialDFAB_gas = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBA_gas = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
                radialDFBB_gas = freud.density.RDF(bins=nBins, r_max=10,r_min=1.0)#h_box-1)    #Set density per bin with a
            
            
                radialDFall_dense.compute(system=system_dense_cut, query_points=bulk_pos , reset=False)               #Calculate radial density function
                radialDFAA_dense.compute(system=system_A_dense_cut, query_points=bulk_pos_A , reset=False)               #Calculate radial density function
                radialDFAB_dense.compute(system=system_B_dense_cut, query_points=bulk_pos_A , reset=False) 
                radialDFBA_dense.compute(system=system_A_dense_cut, query_points=bulk_pos_B , reset=False)               #Calculate radial density function
                radialDFBB_dense.compute(system=system_B_dense_cut, query_points=bulk_pos_B , reset=False) 
                print('7')
            
                radialDFall_edge.compute(system=system_edge_cut, query_points=edge_pos , reset=False)               #Calculate radial density function
                radialDFAA_edge.compute(system=system_A_edge_cut, query_points=edge_pos_A , reset=False)               #Calculate radial density function
                radialDFAB_edge.compute(system=system_B_edge_cut, query_points=edge_pos_A , reset=False) 
                radialDFBA_edge.compute(system=system_A_edge_cut, query_points=edge_pos_B , reset=False)               #Calculate radial density function
                radialDFBB_edge.compute(system=system_B_edge_cut, query_points=edge_pos_B , reset=False) 
                
                radialDFall_gas.compute(system=system_gas_cut, query_points=gas_pos , reset=False)               #Calculate radial density function
                radialDFAA_gas.compute(system=system_A_gas_cut, query_points=gas_pos_A , reset=False)               #Calculate radial density function
                radialDFAB_gas.compute(system=system_B_gas_cut, query_points=gas_pos_A , reset=False) 
                radialDFBA_gas.compute(system=system_A_gas_cut, query_points=gas_pos_B , reset=False)               #Calculate radial density function
                radialDFBB_gas.compute(system=system_B_gas_cut, query_points=gas_pos_B , reset=False) 

g = open(outPath+outTxt2, 'a')
for j in range(0,len(rplot)):
    g.write('{0:.1f}'.format(peA).center(15) + ' ')
    g.write('{0:.1f}'.format(peB).center(15) + ' ')
    g.write('{0:.6f}'.format(rplot[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFall_edge.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAA_edge.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAB_edge.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBA_edge.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBB_edge.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFall_dense.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAA_dense.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAB_dense.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBA_dense.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBB_dense.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFall_gas.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAA_gas.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFAB_gas.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBA_gas.rdf[j]).center(15) + ' ')
    g.write('{0:.6f}'.format(radialDFBB_gas.rdf[j]).center(15) + ' ')
    g.write('{0:.1f}'.format(time_add).center(15) + '\n')
g.close()
