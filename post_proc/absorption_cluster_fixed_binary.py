...
outPath='/Volumes/External/'#'/pine/scr/n/j/njlauers/scm_tmpdir/binary_align/'#'Users/nicklauersdorf/hoomd-blue/build/04_01_20_parent/'#'/Volumes/External/04_01_20_parent/gsd/'
...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    start = 100                  # first frame to process
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            for k in range(1,len(num_sights)):
                ...
                if time_steps_used>0:
                    for ix in range(0, len(occParts)):
                        for iy in range(0, len(occParts)):
                            if losBin[ix][iy]==1:
                                if len(binParts[ix][iy])!=0:
                                    absorb_parts = np.where(((PhaseParts2[binParts[ix][iy]]==0) | (PhaseParts2[binParts[ix][iy]]==1)) & (PhaseParts2_prev[binParts[ix][iy]]==2))
                                    desorb_parts = np.where((PhaseParts2[binParts[ix][iy]]==2) & ((PhaseParts2_prev[binParts[ix][iy]]==0) | (PhaseParts2_prev[binParts[ix][iy]]==1)))
                                    print('absorb')
                                    print(len(absorb_parts))
                                    print('desorb')
                                    print(len(desorb_parts))

            PhaseParts2_prev = np.copy(PhaseParts2)
            time_steps_used +=1
