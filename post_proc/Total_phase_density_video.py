...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/video_frames_new_edge2/'#'Users/nicklauersdorf/hoomd-blue/build/04_01_20_parent/'#'/Volumes/External/04_01_20_parent/gsd/'
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            if steps>0:
                ...
                fig = plt.figure()
                ax = plt.subplot(111)
                scatter = ax.scatter(gas_pos[:,0],gas_pos[:,1], marker='.',label='gas',facecolors='green',edgecolors='green',s=0.5)
                scatter = ax.scatter(bulk_pos[:,0],bulk_pos[:,1], marker='.',label='bulk',facecolors='blue',edgecolors='blue',s=0.5)
                scatter = ax.scatter(edge_pos[:,0],edge_pos[:,1], marker='.',label='edge',facecolors='red',edgecolors='red',s=0.5)
                ...
                plt.title('tauB='+str(round(tst,1))+', edge width='+str(int(bulk_particle_range_final+gas_particle_range))+' lattice spacings, lattice spacing='+str(round(lat,3)))
                #plt.show()
                #stop
                plt.savefig(outPath+'Phase_dens_pa'+str(int(peA))+'_pb'+str(int(peB))+'_eps'+str(eps)+ '_xa'+str(parFrac_orig)+'_fm'+pad+'.png',dpi=150)
                plt.close()
        else:
            ...
            fig = plt.figure()
            ax = plt.subplot(111)
            scatter = ax.scatter(pos[:,0],pos[:,1], marker='.',label='gas',facecolors='green',edgecolors='green',s=0.5)
            ...
            ax.set_xlabel('X-Position')
            ax.set_ylabel('Y-Position')
            plt.title('tauB='+str(round(tst,1))+', edge width=0')
                #plt.show()
                #stop
            plt.savefig(outPath+'Phase_dens_pa'+str(int(peA))+'_pb'+str(int(peB))+'_eps'+str(eps)+ '_xa'+str(parFrac_orig)+'_fm'+pad+'.png',dpi=150)
            plt.close()
