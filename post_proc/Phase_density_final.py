    ...
    for j in range(start, int(end)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            plt.savefig(imgPath+'Cluster_edge_pa'+str(int(peA))+'_pb'+str(int(peB))+'_eps'+str(eps)+ '_xa'+str(int(parFrac))+'_time'+str(tst)+'.png')
            plt.close()
            #plt.show()
            stop
        else:
