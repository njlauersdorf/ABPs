...
dens = os.listdir('../alignment_eps10_phi65')
try:
...
def edge_end_funct(val_arr, rad_arr):
    ...
    if j<len(deriv):
        plt.plot(rad_arr, val_arr)
        plt.plot(rad_arr[:j+1], val_arr[:j+1])
        plt.plot(rad_arr[:len(rad_arr)-1], deriv)
        plt.plot(rad_arr[:j+1], deriv[:j+1])
    else:
        plt.plot(rad_arr, val_arr)
        plt.plot(rad_arr[:j], val_arr[:j])
        plt.plot(rad_arr[:len(rad_arr)-1], deriv)
        plt.plot(rad_arr[:j], deriv[:j])
    plt.show()
    '''
    return j
...
all_dens = []
os.chdir(parent)
os.chdir('../alignment_eps10_phi65')
for i in dens:
#     print(i)
...
for i in range(0, len(all_dens)):
    ...
    if params['eps'][i]==0.0001:
        new_cut_off=int(18*fracd)
    elif params['eps'][i]==0.001:
        new_cut_off=int(26*fracd)
    elif params['eps'][i]==0.01:
        new_cut_off=int(34*fracd)
    elif params['eps'][i]==0.1:
        new_cut_off=int(42*fracd)
    elif params['eps'][i]==1.0:
        new_cut_off=int(50*fracd)
        ...
        if all_dens[i]['clust_size'][times_one*j]>=np.max(all_dens[i]['clust_size'])*0.95:
            for k in range(0,len(angs)):
                ...
                if num_dens[10]>1.0:
                    ...
                    align_orig=smooth(align_orig,7)
                    ...
                    if np.max(alpha_n_int_new)>=0.2:
                        c=edge_end_funct(alpha_n_int_new, rad_new)
                        d=edge_begin_funct(alpha_n_int_new, rad_new)
                        end_rad=rad_new[c]
                        begin_rad=rad_new[d]
                        ...
                        plt.plot(radial_steps_very_orig[begin_rad_orig_ind:end_rad_orig_ind+1], alpha_very_orig[begin_rad_orig_ind:end_rad_orig_ind+1])
                        file_name_final='align_plot_time_'+str(time_list[j])+'_ang_'+str(k*18)+'.png'
                        ...
                        norm_rad_very_orig = np.append(norm_rad_very_orig, 0.0)
                        num_dens_very_orig = np.append(num_dens_very_orig, 0.0)
                        align_very_orig = np.append(align_very_orig, 0.0)
                        alpha_very_orig = np.append(alpha_very_orig, 0.0)
                        
                        norm_rad_end=np.where(norm_rad_very_orig==1.0)[0][0]
                        
                        norm_rad=norm_rad_very_orig[:norm_rad_end+1]
                                    
                        int_len2=norm_rad.max()/10000.0
                        
                        rad_renew=np.arange(0.0, 1.0, int_len2)
