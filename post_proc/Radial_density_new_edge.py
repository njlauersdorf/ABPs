...
outPath='/pine/scr/n/j/njlauers/scm_tmpdir/radial_phase_dens_new_edge/'#'Users/nicklauersdorf/hoomd-blue/build/04_01_20_parent/'#'/Volumes/External/04_01_20_parent/gsd/'
    ...
    for j in range(start, int(end)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            #edge_title="%.2f" % bulk_r_lim_final
            #pad = str(j).zfill(4)
            #fig = plt.figure()
            #ax = plt.subplot(111)
            #scatter = ax.scatter(gas_pos[:,0],gas_pos[:,1],color='green',s=1,label='Gas')
            #scatter = ax.scatter(bulk_pos[:,0],bulk_pos[:,1],color='blue',s=1,label='Bulk')
            #scatter = ax.scatter(edge_pos[:,0],edge_pos[:,1],color='red',s=1,label='Edge')
            #box = ax.get_position()
            ...
            #plt.title('A_edge='+str(count_A_edge_final)+', B_edge='+str(count_B_edge_final)+', edge_width='+str(edge_title)+', tauBrownian='+str(tst))
            #plt.savefig(imgPath+'Cluster_edge_pa'+str(int(peA))+'_pb'+str(int(peB))+'_eps'+str(eps)+ '_xa'+str(int(parFrac))+'_fm'+pad+'.png')
            #plt.close()
            #plt.show()
            binParts = [[[] for b in range(NBins)] for a in range(NBins)]
            typParts=  [[[] for b in range(NBins)] for a in range(NBins)]
            ...
            tmp_posX = pos[:,0] - com_x
            tmp_posY = pos[:,1] - com_y
            ...
            for x in range(0,len(tmp_posX)):
                if tmp_posX[x]>h_box:
                    tmp_posX[x]=tmp_posX[x]-l_box
                elif tmp_posX[x]<(-h_box):
                    tmp_posX[x]=tmp_posX[x]+l_box
                if tmp_posY[x]>h_box:
                    tmp_posY[x]=tmp_posY[x]-l_box
                elif tmp_posY[x]<(-h_box):
                    tmp_posY[x]=tmp_posY[x]+l_box
        
            r_dist_com=(tmp_posX**2+tmp_posY**2)**0.5
