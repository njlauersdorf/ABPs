...
#gsdPath='/Users/nicklauersdorf/hoomd-blue/build/gsd'#/Volumes/External/04_15_20_parent_preset_cluster_lattice_spacing=activity_dependent/gsd'#'/Users/nicklauersdorf/hoomd-blue/build/gsd'
# Run locally
...
peB = float(sys.argv[2])#float(sys.argv[3])
parFrac = float(50)#float(sys.argv[4])
...
with hoomd.open(name=infile, mode='rb') as t:
    ...
    for j in range(start, end):
        ...
        ells0 = [Ellipse(xy=pos0[i,:],
                width=1., height=1., label='PeA: '+str(peA))
        ...
        ax.text(0.95, 0.025, s=r'$\tau_{r}=$' + '{:0.1f}'.format(tst),
        ...
        ax.axes.set_yticks([])
        ax.set_aspect('equal')
        ax.legend(handles=[ells0[0], ells1[1]], labels=['PeA: '+str(peA),'PeB: '+str(peB)], loc='upper right', prop={'size': 15}, markerscale=8.0)
