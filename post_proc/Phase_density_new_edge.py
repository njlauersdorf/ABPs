...
g = open(imgPath2+outTxt, 'w+') # write file headings
    ...
    for j in range(start, int(end)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            count_A_edge_final=0
            count_B_edge_final=0
            count_A_dense_final=0
            count_B_dense_final=0
            count_A_gas_final=0
            count_B_gas_final=0
            gasArea=0
            edgeArea=0
            ...
            fig = plt.figure()
            ax = plt.subplot(111)
            scatter = ax.scatter(pos[:,0],pos[:,1],color='green',s=1,label='All')
            box = ax.get_position()
            ax.set_position([box.x0, box.y0 + box.height*0.15,
                 box.width, box.height * 0.85])
            ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15),
            ...
            ax.set_ylabel('y-position')
            plt.title('A_edge='+str(count_A_edge_final)+', B_edge='+str(count_B_edge_final)+', edge_width='+str(edge_title)+', tauBrownian='+str(tst))
            plt.savefig(imgPath+'Cluster_edge_pa'+str(int(peA))+'_pb'+str(int(peB))+'_eps'+str(eps)+ '_xa'+str(parFrac)+'_fm'+pad+'.png')
            plt.close()
            
            
        
        g = open(imgPath2+outTxt, 'a')
        g.write('{0:.1f}'.format(tst).center(15) + ' ')
        g.write('{0:.3f}'.format(bulk_r_lim_final).center(15) + ' ')
        g.write('{0:.3f}'.format(gas_r_lim).center(15) + ' ')
        g.write('{0:.1f}'.format(end_loop).center(15) + ' ')
        g.write('{0:.3f}'.format(lat).center(15) + ' ')
        g.write('{0:.1f}'.format(count_A_gas_final).center(15) + ' ')
        g.write('{0:.1f}'.format(count_B_gas_final).center(15) + ' ')
        g.write('{0:.1f}'.format(count_A_edge_final).center(15) + ' ')
        g.write('{0:.1f}'.format(count_B_edge_final).center(15) + ' ')
        g.write('{0:.1f}'.format(count_A_dense_final).center(15) + ' ')
        g.write('{0:.1f}'.format(count_B_dense_final).center(15) + ' ')
