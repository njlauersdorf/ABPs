...
with hoomd.open(name=gsdPath+inFile, mode='rb') as t:
    ...
    for j in range(start, int(end/100)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            tot_count_A_edge=np.zeros((end,len(r)))
            tot_count_B_edge=np.zeros((end,len(r)))
            tot_count_A_dense=np.zeros((end,len(r)))
            tot_count_B_dense=np.zeros((end,len(r)))
            tot_count_A_gas=np.zeros((end,len(r)))
            tot_count_B_gas=np.zeros((end,len(r)))
            tot_count_A=np.zeros((end,len(r)))
            tot_count_B=np.zeros((end,len(r)))
            ...
            for i in range(0,len(r)):
                ...
                tot_count_A_edge[j,i]=count_A_edge2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_B_edge[j,i]=count_B_edge2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_A_dense[j,i]=count_A_dense2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_B_dense[j,i]=count_B_dense2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_A_gas[j,i]=count_A_gas2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_B_gas[j,i]=count_B_gas2/(math.pi*((rad_look)**2-(rad_look-1)**2))
                tot_count_A[j,i]=tot_count_A_edge[j,i]+tot_count_A_dense[j,i]+tot_count_A_gas[j,i]
                tot_count_B[j,i]=tot_count_B_edge[j,i]+tot_count_B_dense[j,i]+tot_count_B_gas[j,i]
            #edge=np.min(np.where(tot_count_A[j,:]>tot_count_B[j,:])[0])
            r_norm=r
            
    pad = "pa" + str(peA) + "_pb" + str(peB) + "_xa" + str(parFrac) 
    img_files='/Users/nicklauersdorf/hoomd-blue/build/test6/'
    ...
    open(img_files+"edge_comp_"+pad+"_time.txt","w+")
    open(img_files+"edge_comp_"+pad+"_edge_comp_A_then_b.txt","w+")
    open(img_files+"edge_comp_"+pad+"_gas_comp_A_then_B.txt","w+")
    open(img_files+"edge_comp_"+pad+"_bulk_comp_A_then_B.txt","w+")
    open(img_files+"edge_comp_"+pad+"_edge_area_A_then_b.txt","w+")
    open(img_files+"edge_comp_"+pad+"_gas_area_A_then_B.txt","w+")
    open(img_files+"edge_comp_"+pad+"_bulk_area_A_then_B.txt","w+")
    np.savetxt(img_files+"edge_comp_"+pad+"_time.txt",time_arr,delimiter=',')
    np.savetxt(img_files+"edge_comp_"+pad+"_edge_comp_A_then_B.txt",tot_count_edge,delimiter=',')
    np.savetxt(img_files+"edge_comp_"+pad+"_gas_comp_A_then_B.txt",tot_count_gas, delimiter=',') 
    np.savetxt(img_files+"edge_comp_"+pad+"_bulk_comp_A_then_B.txt",tot_count_dense,delimiter=',')
    np.savetxt(img_files+"edge_comp_"+pad+"_edge_area_A_then_B.txt",edge_area_arr,delimiter=',')
    np.savetxt(img_files+"edge_comp_"+pad+"_gas_area_A_then_B.txt",gas_area_arr, delimiter=',') 
    np.savetxt(img_files+"edge_comp_"+pad+"_bulk_area_A_then_B.txt",dense_area_arr,delimiter=',')
