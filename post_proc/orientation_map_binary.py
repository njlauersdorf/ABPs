        ...
        if len(large_clust_ind_all[0])>0:
            ...
            for ix in range(0, len(binParts)):
                for iy in range(0, len(binParts)):
                    edge_num = np.where(PhaseParts2[binParts[ix][iy]]==0)[0]
                    bulk_num = np.where(PhaseParts2[binParts[ix][iy]]==1)[0]
                    gas_num = np.where(PhaseParts2[binParts[ix][iy]]==2)[0]
                    edge_tot = len(edge_num)
                    bulk_tot = len(bulk_num)
                    gas_tot = len(gas_num)
                    if edge_tot>gas_tot:
                        testIDs_final[ix][iy] = 0
                        PhaseParts2[binParts[ix][iy]]=0
                    elif edge_tot<gas_tot:
                        testIDs_final[ix][iy] = 2
                        PhaseParts2[binParts[ix][iy]]=2
                    else:
                        pass
                    #el
            ...
            binParts = [[[] for b in range(NBins)] for a in range(NBins)]
            typParts=  [[[] for b in range(NBins)] for a in range(NBins)]
            typParts0=  [[[] for b in range(NBins)] for a in range(NBins)]
            typParts1=  [[[] for b in range(NBins)] for a in range(NBins)]
            #posParts=  [[[] for b in range(NBins)] for a in range(NBins)]
            occParts = [[0 for b in range(NBins)] for a in range(NBins)]
            edgeBin = [[0 for b in range(NBins)] for a in range(NBins)]
            ...
            for k in range(0, len(ids)):
                ...
                typParts[x_ind][y_ind].append(typ[k])
                if typ[k]==0:
                    typParts0[x_ind][y_ind].append(typ[k])
                else:
                    typParts1[x_ind][y_ind].append(typ[k])
                #posParts[x_ind][y_ind].append(pos[k])
            ...
            p_all_num = [[0 for b in range(NBins)] for a in range(NBins)]
            ...
            num_dens_0 = [[0 for b in range(NBins)] for a in range(NBins)]
            num_dens_1 = [[0 for b in range(NBins)] for a in range(NBins)]
            
            num_dens_dif = [[0 for b in range(NBins)] for a in range(NBins)]
            num_dens_rat = [[0 for b in range(NBins)] for a in range(NBins)]
                ...
                for iy in range(0, len(occParts)):
                    ...
                    if len(binParts[ix][iy]) != 0:
                        for h in range(0, len(binParts[ix][iy])):
                            ...
                            if PhaseParts2[binParts[ix][iy][h]]!=2:
                        ...
                        num_dens_0[ix][iy] = (len(typParts0[ix][iy])/(sizeBin**2))*(math.pi/4)
                        num_dens_1[ix][iy] = (len(typParts1[ix][iy])/(sizeBin**2))*(math.pi/4)
                        num_dens_dif[ix][iy] = num_dens_1[ix][iy] - num_dens_0[ix][iy]
                        #num_dens_rat[ix][iy] = num_dens_1[ix][iy] / num_dens_0[ix][iy]
                        if p_all_num[ix][iy]>0:
                            p_avg_x[ix][iy] = p_all_x[ix][iy]/p_all_num[ix][iy]
                            p_avg_y[ix][iy] = p_all_y[ix][iy]/p_all_num[ix][iy]
                        else:
                            p_avg_x[ix][iy] = 0.
                            p_avg_y[ix][iy] = 0.
            ...
            plt.scatter(pos[gas_id,0]+h_box, pos[gas_id,1]+h_box, s=1, marker='.', c=red, label='Gas')
...
#                           label=r'$10^{{{}}}$'.format(-i), markersize=msz))
            ...
            plt.legend(handles=[green_patch, yellow_patch, red_patch], fancybox=True, framealpha=0.75, ncol=1, fontsize=21)
            ...
            min_range = im.cvalues.min()
            max_range = im.cvalues.max()
            tot_levels = im.levels
            ...
            im = ax2.contourf(pos_box_x, pos_box_y, num_dens_0)
            
            norm= matplotlib.colors.Normalize(vmin=min_range, vmax=max_range)

            sm = plt.cm.ScalarMappable(norm=norm, cmap = im.cmap)
            sm.set_array([])
            clb = fig.colorbar(sm, ticks=tot_levels, ax=ax2)
            clb.ax.set_title(r'$\phi_a$')
            #fig.colorbar(im, ax=ax2)
            ...
            im = ax2.contourf(pos_box_x, pos_box_y, num_dens_1)
            ...
            im = ax2.contourf(pos_box_x, pos_box_y, num_dens_dif)
            min_range = im.cvalues.min()
            max_range = im.cvalues.max()
            tot_levels = im.levels
            norm= matplotlib.colors.Normalize(vmin=im.cvalues.min(), vmax=im.cvalues.max())
            ...
            sm.set_array([])
            clb = fig.colorbar(sm, ticks=im.levels, ax=ax2)
            clb.ax.set_title(r'$\phi_b-\phi_a$')
            #fig.colorbar(im, ax=ax2)
            ...
            im = ax2.contourf(pos_box_x, pos_box_y, num_dens_rat)
            min_range = im.cvalues.min()
            max_range = im.cvalues.max()
            tot_levels = im.levels
            norm= matplotlib.colors.Normalize(vmin=im.cvalues.min(), vmax=im.cvalues.max())
