...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  9 10:18:38 2020
...
peA = float(sys.argv[1])
peB = float(sys.argv[2])
parFrac = float(sys.argv[3])
...
with hoomd.open(name=gsdPath+inFile, mode='rb') as t:
    ...
    wbinsTrue=(rstop-0.1)/(nBins-1)

    r=np.arange(0.1,rstop,wbinsTrue)
    ...
    for j in range(start, int(end/10)):
        snap = t[j]
        # Easier accessors
        ...
        system_A = freud.AABBQuery(f_box, f_box.wrap(pos0))    #Calculate neighbor list
        system_B = freud.AABBQuery(f_box, f_box.wrap(pos1))    #Calculate neighbor list
...
#        lcID = np.where(clust_size == np.amax(clust_size))
        ...
        rdfAA = freud.density.RDF(bins=nBins, r_max=rstop, r_min=0.1, normalize=True)
        rdfAB = freud.density.RDF(bins=nBins, r_max=rstop, r_min=0.1, normalize=True)
        rdfBA = freud.density.RDF(bins=nBins, r_max=rstop, r_min=0.1, normalize=True)
        rdfBB = freud.density.RDF(bins=nBins, r_max=rstop, r_min=0.1, normalize=True)
        
        rdf.compute(system=system, query_points=pos)
        rdfAA.compute(system=system_A, query_points=pos0)
        rdfAB.compute(system=system_B, query_points=pos0)
        rdfBA.compute(system=system_A, query_points=pos1)
        rdfBB.compute(system=system_B, query_points=pos1)
        
        plt.plot(r,rdf.rdf,label='total')
        plt.plot(r,rdfAA.rdf,label='AA')
        plt.plot(r,rdfAB.rdf,label='AB')
        plt.plot(r,rdfBA.rdf,label='BA')
        plt.plot(r,rdfBB.rdf,label='BB')
        plt.xlabel('r')
        plt.legend()
        plt.xlim(0.1, 10.)
        plt.xlabel(r'$r/a$')
