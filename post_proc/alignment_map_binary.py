...
# Find input area fraction (phi) and time step (dtau)
try:
...
import matplotlib
#matplotlib.use('Agg') #Only use if on the cluster
import matplotlib.pyplot as plt
import matplotlib.collections
from matplotlib.patches import Circle

#Set style choices for plots/figures
...
#Open GSD file to be analyzed and get parameters from
f = hoomd.open(name=inFile, mode='rb')
    ...
    for j in range(start, int(end)):
       ...
       '_ep' + '{0:.3f}'.format(eps)+'_frame'+str(j)
        ...
        if len(large_clust_ind_all[0])>0:
            ...
            # Plot each particle color-coded by phase in addition to the average orientation per bin 
            ...
            if mono_true == 0:
                ...
                # Contour plot of number density for peA
                fig = plt.figure(figsize=(8,6))
                ...
                # Contour plot of number density for peB
                fig = plt.figure(figsize=(8,6))
                ...
                # Contour plot of difference in number density for peA and peB
                fig = plt.figure(figsize=(8,6))
