...
dens = os.listdir('../alignment_eps0001_pe500')
try:
...
def edge_begin_funct(val_arr):
    ...
    if val_max_ind>=5:
        min_range=val_max_ind-5
    else:
        min_range=0
        
    skip=0
    
    if min_range<=deriv_max_ind<=val_max_ind:
        max_slope=deriv[deriv_max_ind]
    elif min_range==val_max_ind:
        skip=1
    else:
        max_slope=np.max(deriv[min_range:val_max_ind])
    ...
    if skip==0:
        ...
        while (((((deriv[i]))>(0.1*max_slope))) or ((val_arr[i]>(0.2*np.max(val_arr))) and (deriv[i]>0.1)) or (val_arr[i]>(0.5*np.max(val_arr)))):
            if val_arr[i]<=0.0:
                #i=i+1
                while_i=1
                break
                
            elif (deriv[i] <=0.0) and (val_arr[i]<=0.2*np.max(val_arr)):
                while_i=1
                #i=i+1
                break
            elif (deriv[i] <=-0.1*max_slope):
                while_i=1
                i=i+1
                break
...
def edge_end_funct(val_arr):
    ...
    print(val_max_ind)
    skip=0
    if ((len(deriv)-5)<=deriv_min_ind<=len(deriv)) or ((len(val_arr)-5)<=val_max_ind<=len(val_arr)):
        print('1')
        j=0.0
        skip=1
    ...
    if skip==1:
        ...
        if j>=len(deriv):
            ...
            while ((((((deriv[j]))>(0.05*min_slope))) and (deriv[j]<0.0)) or ((val_arr[j]>(0.05*np.max(val_arr))) and (deriv[j]<0.0)) or (val_arr[j]>(0.1*np.max(val_arr)))):
            #while ((((deriv[j]))<(0.05*min_slope))) and (val_arr[j]>(0.05*np.max(val_arr))):
                if val_arr[j]<=0.0:
                    break
                    while_j=1
                else:
...
all_dens = []
os.chdir(parent)
os.chdir('../alignment_eps0001_pe500')
for i in dens:
#     print(i)
...
for i in range(0, len(all_dens)):
    ...
    if params['eps'][i]==0.0001:
        new_cut_off=int(15*fracd)
    elif params['eps'][i]==0.001:
        new_cut_off=int(20*fracd)
    elif params['eps'][i]==0.01:
        new_cut_off=int(30*fracd)
    elif params['eps'][i]==0.1:
        new_cut_off=int(28*fracd)
    elif params['eps'][i]==1.0:
        new_cut_off=int(38*fracd)
        ...
        if all_dens[i]['clust_size'][times_one*j]>=np.max(all_dens[i]['clust_size'])*0.95:
            for k in range(0,len(angs)):
                ...
                if num_dens[10]>1.0:
                    ...
                    #if j==280:
                    #    if k==3:
                    #        plt.plot(radial_steps_very_orig, alpha_very_orig)
                    #        plt.show()
                    #        plt.plot(radial_steps, alpha_orig)
                    #        plt.show()
                    ...
                    n_int = interp1d(radial_steps, num_dens, kind='cubic')
                    n_int_new=n_int(rad_new)
                    
                    alpha_int = interp1d(radial_steps, align, kind='cubic')
                    alpha_int_new=alpha_int(rad_new)
                            
                    b=edge_begin_funct(alpha_n_int_new)
                    if b==0.0:
                        pass
                    else:
                        ...
                        c=edge_end_funct(alpha_n_int_new)
                        if c==0.0:
                            pass
                        else:
                        #plt.plot(radial_steps[0:(len(radial_steps)-1)], deriv)
                        #plt.plot(radial_steps[0:(len(radial_steps)-1)], 6*np.abs(deriv)*align[0:(len(radial_steps)-1)])
                        #plt.plot(radial_steps_very_orig, num_dens_very_orig)
                        
                            edge_begin=rad_new[b]
                            edge_end=rad_new[c]
                            
                            
                            
                            if edge_end>edge_begin:
                                edge_end_very_orig_int=find_nearest(radial_steps_very_orig, edge_end)
                                edge_begin_very_orig_int=find_nearest(radial_steps_very_orig, edge_begin)
                                
                                #plt.plot(radial_steps_very_orig, alpha_very_orig)
                                #plt.plot(radial_steps_very_orig[edge_begin_very_orig_int:edge_end_very_orig_int], alpha_very_orig[edge_begin_very_orig_int:edge_end_very_orig_int])
                                #plt.draw()
                                #plt.pause(0.03)
                                #plt.clf()
                                #plt.plot(rad_new, np.abs(n_int_new)*alpha_int_new[0:(len(rad_new))])
                                #plt.plot(rad_new[b:c], (np.abs(n_int_new)*alpha_int_new)[b:c])
                                ...
                                #plt.show()
                                norm_rad_very_orig=(radial_steps_very_orig)/(radial_steps_very_orig[edge_end_very_orig_int])
        
                                int_width = radial_steps_very_orig[edge_end_very_orig_int]-radial_steps_very_orig[edge_begin_very_orig_int]
                                ...
                                '''
                                norm_rad=norm_rad_very_orig
                                norm_alpha_n=alpha_very_orig*num_dens_very_orig
                                norm_align=alpha_very_orig
                                norm_num_dens=num_dens_very_orig
                                
                                int_len2=norm_rad.max()/10000.0
                                rad_renew=np.arange(norm_rad.min(), norm_rad.max(), int_len2)
                                #print(rad_renew)
                                #print(np.size(rad_renew))
                                alpha_n_int_renew = interp1d(norm_rad_very_orig, alpha_very_orig, kind='cubic')
                                align_int_renew = interp1d(norm_rad_very_orig, align_very_orig, kind='cubic')
                                num_dens_int_renew = interp1d(norm_rad_very_orig, num_dens_very_orig, kind='cubic')
                                alpha_n_int_rerenew=alpha_n_int_renew(rad_renew)
                                align_int_rerenew=align_int_renew(rad_renew)
                                num_dens_int_rerenew=num_dens_int_renew(rad_renew)
                                ...
                                int_begin_avg+=radial_steps_very_orig[edge_begin_very_orig_int]
                                int_end_avg+=radial_steps_very_orig[edge_end_very_orig_int]
                            else:
                                pass
    lat=getLat(params['pe'][i],params['eps'][i])
    int_width_tot=int_width_avg
    '''
    plt.figure()
