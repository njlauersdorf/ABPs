...
for i in range(0, len(all_dens)):
    ...
    outTxt2 = 'align_single_pa'+str(params['peA'][i])+'_pb'+str(params['peB'][i])+'_phi'+str(params['phi'][i])+'_eps'+str(params['eps'][i])+'.txt'
    outTxt3 = 'align_single_interface_only_pa'+str(params['peA'][i])+'_pb'+str(params['peB'][i])+'_phi'+str(params['phi'][i])+'_eps'+str(params['eps'][i])+'.txt'
    
    g = open(outTxt2, 'w+') # write file headings
    g.write('pa'.center(15) + ' ' +\
        'pb'.center(15) + ' ' +\
        'xa'.center(15) + ' ' +\
        'phi'.center(15) + ' ' +\
        'eps'.center(15) + ' ' +\
    ...
    for j in range(0,times-1):
        ...
        if all_dens[i]['clust_size'][times_one*j]>=np.max(all_dens[i]['clust_size'])*0.95:
            for k in range(0,len(angs)):
                ...
                num_dens_0=pd.Series.to_numpy(all_dens[i]['num_dens_0'][new_cut_off+(times_one*j+k*len(rad_len)):(times_one*j+(k+1)*len(rad_len))])
                num_dens_1=pd.Series.to_numpy(all_dens[i]['num_dens_1'][new_cut_off+(times_one*j+k*len(rad_len)):(times_one*j+(k+1)*len(rad_len))])

                
                
                num_dens=smooth(num_dens,3)
                num_dens_0=smooth(num_dens_0,3)
                num_dens_1=smooth(num_dens_1,3)
