...
sigA = ${sig_a}                   # activity of A particles
sigB = ${sig_b}                   # activity of B particles
...
D_rA = (3.0 * D_t) / (sigA**2)  # rotational diffusion constant
D_rB = (3.0 * D_t) / (sigB**2)  # rotational diffusion constant

tauBrown = (sigma**2) / D_t     # brownian time scale (invariant)

def computeVel(activity, sig, dr):
    "Given particle activity, output intrinsic swim speed"
    # This gives:
    # v_0 = Pe * sigma / tau_B = Pe * sigma / 3 * tau_R
    velocity = (activity * sig) / (3 * (1/dr))
    return velocity
...
if peB != 0:                        # B particles are NOT Brownian
    vB = computeVel(peB, sigB, D_rB)
    FpB = computeActiveForce(vB)
    epsB=eps
    #epsB = computeEps(alpha, FpB)
    #epsB = kT
    tauB = computeTauLJ(epsB, sigB)
