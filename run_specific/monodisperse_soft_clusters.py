...
dt = 0.0000001 * tauLJ                        # timestep size
...
phiG = compPhiG(phi, peS, peF, xF, latF, latS)
# Compute the fraction of particles in the dense phase
cf = clustFrac(phi, phiG, latF, latS, xF)
# Multiply by N...
Nl = int(cf * partNum)
#Nls = int(Nl * (1. - xF))
#Nlf = Nl - Nls
...
Al = ((Nl * np.pi * (latNet**2) * 0.25)) / phiCP

# The area for seed
Al_real=Al/4
print((Al_real*phiCP)/(np.pi*(latNet**2)*0.25))
...
out = "cluster_pa" + str(int(peList[0]))
out += "_pb" + str(int(peList[1]))
out += "_phi" + str(intPhi)
out += "_eps" + str(eps)
out += "_xa" + str(xF)
