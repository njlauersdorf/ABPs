...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    for p in range(start, end):     
        ...
        pe_avg_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        pe_tot=[[0 for b in range(NBins)] for a in range(NBins)]
        pe_sum=[[0 for b in range(NBins)] for a in range(NBins)]
        binArea = sizeBin**2
        #Calculate weighted alignment/num density product for determining highly aligned interface
        for ix in range(0, len(occParts)):
                for iy in range(0, len(occParts)):
                    for h in range(0, len(binParts[ix][iy])):
                        if typ[binParts[ix][iy][h]]==0:
                            ...
                            pe_sum[ix][iy] +=1  
                    if pe_sum[ix][iy] > 0:
                        pe_avg_arr[ix][iy] = pe_tot[ix][iy]/pe_sum[ix][iy]               
                        press_int[ix][iy] += ((num_dens3[ix][iy] * pe_avg_arr[ix][iy] * new_align_avg[ix][iy])) * binArea * sizeBin
                    #press_int[ix][iy] = (num_dens3[ix][iy] * new_align_avg[ix][iy])
            
                    #press_int[ix][iy] = num_dens3[ix][iy]*new_align_avg[ix][iy]
                    press_intA[ix][iy] = num_dens3A[ix][iy]*new_align_avg0[ix][iy] * peA
                    press_intB[ix][iy] = num_dens3B[ix][iy]*new_align_avg1[ix][iy] * peB
        ...
        SigXX_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        SigXY_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        SigYX_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        SigYY_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        for ix in range(0, len(occParts)):
                for iy in range(0, len(occParts)):
                    for h in binParts[ix][iy]:
                        tmp_posX = pos[h][0] + h_box
                        tmp_posY = pos[h][1] + h_box
                        x_ind = int(tmp_posX / sizeBin)
                        y_ind = int(tmp_posY / sizeBin)
                        if (x_ind + 1) != NBins:
                            lookx = [x_ind-1, x_ind, x_ind+1]
                        elif (x_ind) == 0:
                            lookx = [NBins-1, x_ind, x_ind+1]
                        else:
                        ...
                        for indx in lookx:
                            ...
                            for indy in looky:
                                ...
                                for comp in binParts[indx][indy]:
                                    difx = pos[comp][0]-refx
                                    dify = pos[comp][1]-refy
                                    ...
                                    if 0.1 < dist <= r_cut:
                                        ...
                                        SigXX_arr[ix][iy] += (fx * difx)/2
                                        SigYY_arr[ix][iy] += (fy * dify)/2
                                        SigXY_arr[ix][iy] += (fx * dify)/2
                                        SigYX_arr[ix][iy] += (fy * difx)/2        
        
        
        
        Press_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        pe_avg_arr = [[0 for b in range(NBins)] for a in range(NBins)]
        pe_tot=[[0 for b in range(NBins)] for a in range(NBins)]
        pe_sum=[[0 for b in range(NBins)] for a in range(NBins)]
        fa_all_avg = [[0 for b in range(NBins)] for a in range(NBins)]
        fa_fast_avg = [[0 for b in range(NBins)] for a in range(NBins)]
        fa_slow_avg = [[0 for b in range(NBins)] for a in range(NBins)]
        bulk_interpart_press_sum = 0
        bulk_interpart_press_num = 0
        for ix in range(0, len(occParts)):
            for iy in range(0, len(occParts)):
                Press_arr[ix][iy] = (SigXX_arr[ix][iy] + SigYY_arr[ix][iy]) / (2*binArea)
                if phaseBin[ix][iy]==0:
        ...
        if bulk_interpart_press_num > 0:
            bulk_interpart_press_avg = bulk_interpart_press_sum / bulk_interpart_press_num
        else:
            bulk_interpart_press_avg = 0
        
        for ix in range(0, len(occParts)):
            for iy in range(0, len(occParts)):
                if phaseBin[ix][iy]==0:
                    bulk_interpart_press_std_sum += (Press_arr[ix][iy] - bulk_interpart_press_avg)**2
                    bulk_interpart_press_std_num += 1
        if bulk_interpart_press_std_num > 0:
