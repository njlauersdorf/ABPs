...
class run_sim:
    def __init__(self, hoomdPath, runFor, dumpFreq, partPercA, peA, peB, partNum, intPhi, eps, aspect_ratio, seed1, seed2, seed3, seed4, seed5):
        ...
        self.numDumps = float(self.simLength / 0.05)           # dump data every 0.1 tauBrown.
    ...
    def fast_penetrate_immobile_membrane(self):
        ...
        if self.length != self.width:
            ...
            # Compute lattice spacing based on each activity
            latNet = 1.9*self.theory_functs.conForRClust2(self.peA, self.peB, self.beta_A, self.beta_B, self.eps)
