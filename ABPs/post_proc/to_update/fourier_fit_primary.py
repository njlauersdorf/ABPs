...
Method: Uses primary method
(scipy.optimize.curve_fit) for fitting simulation data [see fourier_fit_clust.py for 
secondary method].
    ...
    #If not initial row number
            ...
            #If row belongs to a higher time step, we want to fit previous time's data
                ...
                if len(ang)>0:
                    ...
                    #Fit des_ang and des_rad to Fourier series defined by fourier()
                    popt, pcov = curve_fit(fourier, des_ang, des_rad, [1.0] * n_len*2)
