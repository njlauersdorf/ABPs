...
python3 $script_path/Sim_frame_image_com_updated.py $fname $pa2 $pb $xa $ep $phi $dtau $hoomd_path $txt_path $pic_path
...
#Start
  ...
  ffmpeg -start_number 1 -framerate 10 -i "$pic_path"sim_frame_pa${pa2}_pb${pb}_xa50_eps${eps}_phi${phi}_pNum${pNum}_frame_%04d.png\
 -vcodec libx264 -s 1600x1200 -pix_fmt yuv420p -threads 1\
 "$vid_path"sim_frame_pa${pa2}_pb${pb}_xa50_eps${eps}_phi${phi}_pNum${pNum}.mp4

  rm -rf "$pic_path"sim_frame_pa${pa2}_pb${pb}_xa50_eps${eps}_phi${phi}_pNum${pNum}_frame_*
