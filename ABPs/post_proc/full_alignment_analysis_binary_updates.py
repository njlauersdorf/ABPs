...
with hoomd.open(name=inFile, mode='rb') as t:
    ...
    for p in range(start, end):     
        ...
        div_min = -3
        min_n = -np.max(np.abs(new_align_avg))
        max_n = np.max(np.abs(new_align_avg))
        levels_text=40
        level_boundaries = np.linspace(min_n, max_n, levels_text + 1)
        im = plt.contourf(pos_box_x, pos_box_y, new_align_avg, level_boundaries, vmin=min_n, vmax=max_n, cmap='seismic', extend='both')
...
values=(level_boundaries[:-1] + level_boundaries[1:]) / 2, format=tick.FormatStrFormatter('%.2f'))
        ...
        plt.tight_layout()
        plt.savefig(outPath + 'align_' + out + pad + ".png", dpi=100)
        plt.close()
        ...
        levels_text=40
        level_boundaries = np.linspace(min_n, max_n, levels_text + 1)
        im = plt.contourf(pos_box_x, pos_box_y, new_align_avg_trad, level_boundaries, vmin=min_n, vmax=max_n, cmap='seismic', extend='both')
        norm= matplotlib.colors.Normalize(vmin=min_n, vmax=max_n)
        #plt.plot(xn_pos, yn_pos, c='orange', lw=4.0)
        #if interior_bin>0:
        if bub_large >=1:
...
values=(level_boundaries[:-1] + level_boundaries[1:]) / 2, format=tick.FormatStrFormatter('%.2f'))
        ...
        ax.axis('off')
        plt.tight_layout()
        plt.savefig(outPath + 'aligntrad_' + out + pad + ".png", dpi=100)
        plt.close()
        ...
        levels_text=40
        level_boundaries = np.linspace(min_n, max_n, levels_text + 1)
        im = plt.contourf(pos_box_x, pos_box_y, new_align_avg0, level_boundaries, vmin=min_n, vmax=max_n, cmap='seismic', extend='both')
        norm= matplotlib.colors.Normalize(vmin=min_n, vmax=max_n)
        #plt.plot(xn_pos, yn_pos, c='orange', lw=4.0)
        #if interior_bin>0:
        if bub_large >=1:
...
values=(level_boundaries[:-1] + level_boundaries[1:]) / 2, format=tick.FormatStrFormatter('%.2f'))
        ...
        ax.axis('off')
        plt.tight_layout()
        plt.savefig(outPath + 'alignA_' + out + pad + ".png", dpi=100)
        plt.close()
        ...
        levels_text=40
        level_boundaries = np.linspace(min_n, max_n, levels_text + 1)
        im = plt.contourf(pos_box_x, pos_box_y, new_align_avg1, level_boundaries, vmin=min_n, vmax=max_n, cmap='seismic', extend='both')
        norm= matplotlib.colors.Normalize(vmin=min_n, vmax=max_n)
        #plt.plot(xn_pos, yn_pos, c='orange', lw=4.0)
        #if interior_bin>0:
        if bub_large >=1:
...
values=(level_boundaries[:-1] + level_boundaries[1:]) / 2, format=tick.FormatStrFormatter('%.2f'))
        ...
        plt.text(00.663, 0.04, s=r'$\tau$' + ' = ' + '{:.1f}'.format(3*tst) + ' ' + r'$\tau_\mathrm{r}$',
                fontsize=18, transform = ax.transAxes,
                bbox=dict(facecolor=(1,1,1,0.75), edgecolor=(0,0,0,1), boxstyle='round, pad=0.1'))

        ax.axis('off')
        plt.tight_layout()
        plt.savefig(outPath + 'alignB_' + out + pad + ".png", dpi=100)
        plt.close()
        
        if np.abs(np.max(new_align_avg_dif)) > np.abs(np.min(new_align_avg_dif)):
            min_n = -np.abs(np.max(new_align_avg_dif))
            max_n = np.abs(np.max(new_align_avg_dif))
        else:
            min_n = -np.abs(np.min(new_align_avg_dif))
            max_n = np.abs(np.min(new_align_avg_dif))
        ...
        levels_text=40
        level_boundaries = np.linspace(min_n, max_n, levels_text + 1)
        im = plt.contourf(pos_box_x, pos_box_y, new_align_avg_dif, level_boundaries, vmin=min_n, vmax=max_n, cmap='seismic', extend='both')
        norm= matplotlib.colors.Normalize(vmin=min_n, vmax=max_n)
        #plt.plot(xn_pos, yn_pos, c='orange', lw=4.0)
        #if interior_bin>0:
        if bub_large >=1:
...
values=(level_boundaries[:-1] + level_boundaries[1:]) / 2, format=tick.FormatStrFormatter('%.2f'))
        clb.ax.tick_params(labelsize=16)
        clb.set_label(r'$\alpha_\mathrm{B}-\alpha_\mathrm{A}$', labelpad=-40, y=1.07, rotation=0, fontsize=20)
        plt.quiver(pos_box_x, pos_box_y, align_norm_x, align_norm_y, color='black')
        ...
        ax.axis('off')
        plt.tight_layout()
        #plt.gcf().subplots_adjust(top=0.01)
        plt.savefig(outPath + 'alignDif_' + out + pad + ".png", dpi=100)
        plt.close()
        '''
        ...
        if len(bub2_parts)>0:
            plt.scatter(pos[bub2_parts,0]+h_box, pos[bub1_parts,1]+h_box, s=0.75, marker='.', c=purple) 
        if len(bub3_parts)>0:
            plt.scatter(pos[bub3_parts,0]+h_box, pos[bub1_parts,1]+h_box, s=0.75, marker='.', c=purple) 
        if len(bub4_parts)>0:
            plt.scatter(pos[bub4_parts,0]+h_box, pos[bub1_parts,1]+h_box, s=0.75, marker='.', c=purple) 
        if len(bub5_parts)>0:
            plt.scatter(pos[bub5_parts,0]+h_box, pos[bub1_parts,1]+h_box, s=0.75, marker='.', c=purple) 
        if bub_large >=1:
            if interior_bin>0:
                plt.scatter(xn_pos, yn_pos, c='black', s=3.0)
            if exterior_bin>0:
