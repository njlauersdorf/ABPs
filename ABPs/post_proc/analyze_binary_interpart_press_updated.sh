...
python3 $script_path/interpart_press_updated.py $fname $pa2 $pb $xa $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
...
ffmpeg -start_number 0 -framerate 10 -i "$pic_path"interpart_press_pa${pa2}_pb${pb}_xa${xa}_eps${eps}_phi${phi}_pNum${pNum}_bin${bin}_time${step}_frame_%04d.png\
 -vcodec libx264 -s 1600x1200 -pix_fmt yuv420p -threads 1\
 "$vid_path"interpart_press_pa${pa2}_pb${pb}_xa${xa}_eps${eps}_phi${phi}_pNum${pNum}_bin${bin}_time${step}.mp4

  rm -rf "$pic_path"interpart_press_pa${pa2}_pb${pb}_xa${xa}_eps${eps}_phi${phi}_pNum${pNum}_bin${bin}_time${step}_frame_*
