...
typ0ind=np.where(np.array(typ)==0)[0]       # Calculate which particles are type 0
typ1ind=np.where(np.array(typ)==1)[0]       # Calculate which particles are type 1

x,y,z = zip(*pos)

pos0x=np.array(x)[typ0ind.astype(int)]                               # Find positions of type 0 particles
pos1x=np.array(x)[typ1ind.astype(int)]

pos0y=np.array(y)[typ0ind.astype(int)]                               # Find positions of type 0 particles
pos1y=np.array(y)[typ1ind.astype(int)]
