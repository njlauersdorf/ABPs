...
if [ "$method" = "interface_pressure_com" ]; then
    python3 $script_path/src/post_proc/align_pressure_CoM.py $fname $pa2 $pb $xa2 $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
elif [ "$method" = "interface_pressure_surface" ]; then
    python3 $script_path/src/post_proc/align_pressure_updated.py $fname $pa2 $pb $xa2 $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
elif [ "$method" = "bulk_pressure_total" ]; then
    python3 $script_path/src/post_proc/interparticle_pressure_true2.py $fname $pa2 $pb $xa2 $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
elif [ "$method" = "bulk_pressure_phases" ]; then
    python3 $script_path/src/post_proc/interpart_press_updated.py $fname $pa2 $pb $xa2 $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
elif [ "$method" = "lattice_spacing" ]; then
    python3 $script_path/src/post_proc/lattice_spacing.py $fname $pa2 $pb $xa2 $ep $phi $dtau $bin $step $hoomd_path $txt_path $pic_path
